...
void evaluation_fonction(fmpz_t res, fmpz_mod_poly_t fonction,long valeur_x){
	...
	for (long i=1;i<length;i++){
		...
		if(fmpz_get_ui(temp_coef)!=0){ //si la valeur du coeficient est différent de 
...
/**
	Additionne deux polynômes. Vu que nous somme dans le champs gallois,
	 ceci revient a faire le zor de chaque coefficient
*/
...
void xor_polynome(fmpz_mod_poly_t resu, fmpz_mod_poly_t poly1_v, fmpz_mod_poly_t poly2_v){
	...
	// on boucle le degré de polynome le plus grand
	if(deg1<=deg2){
...
/**
	Cette fonction multipli un polynome et un valeur et elle est utilisée dans la fonction de division 
*/
...
void mulPolyInDiv(fmpz_mod_poly_t res,fmpz_mod_poly_t pol2,fmpz_t valeur, long degre_poly )
{
	...
	for(long i=0; i<=deg;i++){ 
		...
		if(fmpz_get_ui(temp)!=0){
			//Récupérer les valeurs des index du nombre a calculer et celui du coefficient à la position i, puis les additionner
			fmpz_add(temp,ptoi[fmpz_get_ui(temp)],ptoi[fmpz_get_ui(valeur)]); 
			//calculer le modulo gf-1
			fmpz_mod_ui(temp,temp,gf-1);  
			//obtenir la valuer en decimal de l'élément du corprs galois qui est à la position calculé
			fmpz_mod_poly_evaluate_fmpz(temp,tab[fmpz_get_ui(temp)],d);
		}
		// stocker le résultat
...
void division(fmpz_mod_poly_t q, fmpz_mod_poly_t r, fmpz_mod_poly_t dividente, fmpz_mod_poly_t deviseur){
	...
	fmpz_mod_poly_init(poly_temp, n);
	//si le degré de la dividente est plus petite que le degré du diviseur
	if (degr0<degr1){ 
	...
	//si le degré de la dividente est plus grande que le degré du diviseur
	else{

		//Faire tant que le degré de la dividante est plus petite que celui du diviseur 
...
void algo_euclide( fmpz_mod_poly_t localisation,fmpz_mod_poly_t amplitude , fmpz_mod_poly_t x2t_v, fmpz_mod_poly_t syndrome_v,long tt){
	...
	while(deg_reste>(tt/2)|| i==2) {  //tant que le degré 

		fmpz_mod_poly_init(reste,n);
		fmpz_mod_poly_init(quotient,n);
		fmpz_mod_poly_init(poly_temp,n);

		division(quotient,reste, x2t, synd); //effectue la division 
		mulPoly(multi,ampl, quotient); // multplie la dividente par le quotient trouvé
		xor_polynome(poly_temp ,multi, local ); // additon le résultat de la multiplication avec la dividente
